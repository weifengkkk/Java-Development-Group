 ...
 * Summary: A polyfill for CSS com.soft.filter effects
 * License: MIT
 * Version: 0.22
 *
 * URL:
				...
				if(declaration.property === 'filter'){
					...
					if(properties['filtersW3C'].length > 0){
						...
						if(typeof this._ie === 'undefined'){
							newstyles += '-ms-com.soft.filter:' + msFilter + ';';
						}
						
						newstyles += '-webkit-com.soft.filter:' + webkitFilter + ';';
						newstyles += '-moz-com.soft.filter:' + mozFilter + ';';
						newstyles += '-o-com.soft.filter:' + oFilter + ';';
					}
					if(properties['filtersSVG'].length > 0){
						if(properties['filtersSVG'][0] != 'none'){
							...
							if(typeof XMLSerializer === 'undefined'){
								newstyles += 'com.soft.filter: url(#' + id + ')';
							}
							else {
								var s = new XMLSerializer();
								var svgString = s.serializeToString(this._svg_cache[id]);
								...
								if(svgString.search('SourceGraphic') != -1){
									...
									newstyles += 'com.soft.filter: url(\'data:image/svg+xml;utf8,' + svgString + '#' + id + '\')';
								}
							}
						}
						else {
							newstyles += 'com.soft.filter: none;';
						}
					}
					if(typeof this._ie !== 'undefined'){
						if(properties['filtersIE'].length > 0){
							...
							newstyles += 'com.soft.filter:' + filtersIE + ';';
						}
